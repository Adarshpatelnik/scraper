...
const TransactionReport = () => {
  ...
  const columnDefs = [
    ...
    { headerName: 'Trans Written Premium', field: 'Trans Written Premium', cellRenderer: currencyFormatter, filter: true, floatingFilter: true },
    { headerName: 'Federal Policy Fee', field: 'Federal Policy Fee', cellRenderer: currencyFormatter, filter: true, floatingFilter: true },
    { headerName: 'HFIAA Surcharge', field: 'HFIAA Surcharge', cellRenderer: currencyFormatter, filter: true, floatingFilter: true },
    { headerName: 'Reserve Assessment Fund', field: 'Reserve Assessment Fund', cellRenderer: currencyFormatter, filter: true, floatingFilter: true },
  ...
  const printData = () => {
    ...
    printWindow.document.write('<style> .ag-header-cell { background-color: #000080; color: white; } </style>'); 
    printWindow.document.write('</head><body >');
    printWindow.document.write('<h1>Transaction Report</h1>');
    printWindow.document.write('<div id="printTable">');
    printWindow.document.write(document.querySelector('.ag-theme-alpine').innerHTML);
  ...
  return (
    <Container>
      <ButtonContainer>
        <Button onClick={downloadCSV}><i className="bi bi-file-earmark-spreadsheet"></i> Download CSV</Button>
        <Button onClick={printData}><i className="bi bi-printer"></i> Print</Button>
        <Button onClick={exportPDF}><i className="bi bi-file-earmark-pdf"></i> Export PDF</Button>
      </ButtonContainer>
